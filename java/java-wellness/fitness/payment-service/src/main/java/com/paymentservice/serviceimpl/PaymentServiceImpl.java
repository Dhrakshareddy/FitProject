...
public class PaymentServiceImpl implements PaymentService {
	...
	public void save(Payment payment) {
		...
		ResponseEntity<UserBean> responseEntity = restTemplate.exchange(
				"http://13.48.82.196:8402/api/user/getByName/" + payment.getUsername(), HttpMethod.GET, httpEntity,
				UserBean.class);
	...
	public ResponseDto getPayments(Long paymentId) {
		...
		HttpEntity<String> httpEntity = new HttpEntity<String>(headers);
		ResponseEntity<UserBean> responseEntity = restTemplate.exchange(
				"http://13.48.82.196:8402/api/user/getById/" + payment.getUserId(), HttpMethod.GET, httpEntity,
				UserBean.class);
		UserBean userBean = responseEntity.getBody();
