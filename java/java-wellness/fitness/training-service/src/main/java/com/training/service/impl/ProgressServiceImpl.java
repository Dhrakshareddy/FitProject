...
public class ProgressServiceImpl implements ProgressService {
	...
	public void save(Progress progress) {
		...
		ResponseEntity<UserBean> responseEntity = restTemplate.exchange(
				"http://13.48.82.196:8402/api/user/getByName/" + progress.getUsername(), HttpMethod.GET, httpEntity,
				UserBean.class);
	        ...
	        for (Workout workout : workouts) {
	            ...
	            if (exercise.getUsername().equals(workout.getUsername())) {
	                if (exercise.getNumberOfSets() > 0) {
	                    double exerciseProgress = (double) workout.getSetsCompleted() / exercise.getNumberOfSets() * 100.0;
